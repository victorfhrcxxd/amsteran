...
public final class SelectorThread<T extends MMOClient<?>> extends Thread
{
	...
	*/
	...
	private final void acceptConnection(final SelectionKey key, MMOConnection<T> con)
	{
		...
		try
		{
			while ((sc = ssc.accept()) != null)
			{
				...
				{
					/*
					if (IPProtection(sc.socket().getInetAddress().getHostAddress()))
					{
						sc.socket().close();
						continue;
					}
					
					if (Config.GAME_FLOOD_PROTECTION && sc.socket().getLocalPort() == Config.PORT_GAME)
					{
						ForeignConnection fConnection = _floodProtection.get(sc.socket().getInetAddress().getHostAddress());
						if (fConnection != null)
						{
							fConnection.connectionNumber++;
							if (fConnection.connectionNumber > Config.GAME_FAST_CONNECTION_LIMIT && System.currentTimeMillis() - fConnection.lastConnection < Config.GAME_NORMAL_CONNECTION_TIME || System.currentTimeMillis() - fConnection.lastConnection < Config.GAME_FAST_CONNECTION_TIME || fConnection.connectionNumber > Config.GAME_MAX_CONNECTION_PER_IP)
							{
								fConnection.lastConnection = System.currentTimeMillis();
								System.out.println("------------------------------------------------------------------------------");
								System.out.println("WARN: Flood Detectado e Boqueado! ~~ [-" + sc.socket().getInetAddress().getHostAddress() + "-]");
								System.out.println("------------------------------------------------------------------------------");
							...
							if (fConnection.isFlooding)
							{
								...
								System.out.println((new StringBuilder()).append(sc.socket().getInetAddress().getHostAddress()).append(" is not considered as flooding anymore.").toString());
							}
							fConnection.lastConnection = System.currentTimeMillis();
							fConnection = null;
						}
						else
						{
							...
							_floodProtection.put(sc.socket().getInetAddress().getHostAddress(), fConnection);
							fConnection = null;
						}
					}
					*/
	...
	/*
	public synchronized static boolean IPProtection(String ip)
	{
		boolean result = true;
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("SELECT ip FROM ip_protection WHERE ip=?");
		...
		{
			System.out.println("SelectorThread -> IPProtection: " + e.getMessage());
		}
		return result;
	}
	*/
