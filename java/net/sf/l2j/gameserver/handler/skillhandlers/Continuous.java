		...
		for (L2Object obj : targets)
		{
			...
			switch (skill.getSkillType())
			{
				case BUFF:
					...
					if (!(activeChar instanceof L2PcInstance && target instanceof L2PcInstance && party != null && party.getPartyMembers().contains(target)))
					{
						// Anti-Buff Protection prevents you from getting buffs by other players
						if (activeChar instanceof L2PcInstance && target instanceof L2PcInstance && target != activeChar && target.isBuffProtected() && !skill.isHeroSkill() && (skill.getSkillType() == L2SkillType.BUFF && !(skill.getId() == 1323 || skill.getId() == 1410 || skill.getId() == 1418)))
						{
							((L2PcInstance) activeChar).sendMessage(target.getName() + " is with the skill Anti-Buff active!");
							continue;
						}
					}
