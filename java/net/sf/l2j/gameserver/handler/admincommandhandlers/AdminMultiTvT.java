...
import net.sf.l2j.gameserver.model.entity.events.multiteamvsteam.MultiTvTEventTeleporter;
...
public class AdminMultiTvT implements IAdminCommandHandler
{
    ...
    {
        if (command.startsWith("admin_mtvt_add"))
        {
        ...
        else if (command.startsWith("admin_mtvt_remove"))
        {
            L2Object target = activeChar.getTarget();
        ...
        else if (command.startsWith("admin_mtvt_advance"))
        {
            MultiTvTManager.getInstance().skipDelay();
        }
    ...
    {
        if (MultiTvTEvent.isPlayerParticipant(playerInstance.getObjectId()))
        {
            activeChar.sendMessage("Player already participated in the event!");
            return;
        }
       
        if (!MultiTvTEvent.addParticipant(playerInstance))
        {
            activeChar.sendMessage("Player instance could not be added, it seems to be null!");
            return;
        }
       
        if (MultiTvTEvent.isStarted())
        {
            new TvTEventTeleporter(playerInstance, MultiTvTEvent.getParticipantTeamCoordinates(playerInstance.getObjectId()), true, false);
        }
    }
   
    private void remove(L2PcInstance activeChar, L2PcInstance playerInstance)
    {
        if (!MultiTvTEvent.removeParticipant(playerInstance.getObjectId()))
        {
            activeChar.sendMessage("Player is not part of the event!");
            return;
        }
       
        new MultiTvTEventTeleporter(playerInstance, MultiTvTConfig.MULTI_TVT_EVENT_PARTICIPATION_NPC_COORDINATES, true, true);
    }
}