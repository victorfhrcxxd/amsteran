...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_getinstance"))
		{
			...
			L2PcInstance player = L2World.getInstance().getPlayer(target_name);
			if(player == null)
			{
				activeChar.sendMessage("Player is offline");
				return false;
			...
			activeChar.setNewInstance(player.getNewInstance(), false);
		...
		{
			activeChar.setNewInstance(InstanceManager.getInstance().getInstance(0), false);
