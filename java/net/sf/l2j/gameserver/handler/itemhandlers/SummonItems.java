...
public class SummonItems implements IItemHandler
{
	...
	public void useItem(L2Playable playable, ItemInstance item, boolean forceUse)
	{
		...
		if (!MultiTvTEvent.onItemSummon(playable.getObjectId()) || !TvTEvent.onItemSummon(playable.getObjectId()) || !LMEvent.onItemSummon(playable.getObjectId()) || !DMEvent.onItemSummon(playable.getObjectId()) || !KTBEvent.onItemSummon(playable.getObjectId()))
			return;
		...
		if (activeChar.inObserverMode() || activeChar.isOlympiadProtection())
			return;
			...
			case 0: // static summons (like Christmas tree)
				try
				{
					...
					else
					{
						final L2Spawn spawn = new L2Spawn(npcTemplate);
						spawn.setHeading(-1);
						spawn.setLocx(activeChar.getX() + 25);
						spawn.setLocy(activeChar.getY() + 35);
						spawn.setLocz(activeChar.getZ() + 20);
						spawn.stopRespawn();
						...
						if (npc instanceof L2AgathionInstance)
						{
							...
							((L2AgathionInstance) npc).setShowSummonAnimation(true);
							((L2AgathionInstance) npc).setOwner(activeChar);
							((L2AgathionInstance) npc).setIsInvul(true);
							((L2AgathionInstance) npc).setRunning();
							activeChar.setAgathion(((L2AgathionInstance) npc));
							activeChar.setAgathionId(((L2AgathionInstance) npc).getNpcId());
							if (Config.AGATHIONS_ENABLED && Util.contains(Config.AGATHIONS_LIST_ID, ((L2AgathionInstance) npc).getNpcId()))
							{
