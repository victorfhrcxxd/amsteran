...
public class DailyRewardManager
{
	...
	private static void giveReward(L2PcInstance player)
	{
		...
		for (RewardHolder reward : Config.DAILY_LOG_REWARDS)
		{
			...
			{
				if (player.getInventory().getItemByItemId(reward.getRewardId()) == null)
				{
					player.addItem("Daily Reward", reward.getRewardId(), Rnd.get(reward.getRewardMin(), reward.getRewardMax()), player, true);
					/*
					player.setTimedItens(System.currentTimeMillis());
					player.store();
					ThreadPoolManager.getInstance().scheduleGeneral(new TaskItemDestroy(player), Config.DAILY_REWARDS_DELETE_TIME * 1000 * 60);
					*/
				}
			}
		}
	...
	private static void insertNewParentOfPlayerIPHWID(L2PcInstance player)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			statement = con.prepareStatement("INSERT INTO reward_manager (hwid,expire_time) VALUES (?,?)");
