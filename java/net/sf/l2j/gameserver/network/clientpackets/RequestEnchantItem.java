...
public final class RequestEnchantItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		// template for scroll
		final L2EnchantScroll enchant = EnchantTable.getInstance().getEnchantScroll(scroll);
		if (enchant == null)
			return;
		...
		synchronized (item)
		{
			if (Rnd.get(100) < enchant.getChance(item)) // success
			{
				...
				if (enchant.announceTheEnchant(item))
					Broadcast.ServerAnnounce(String.format(enchant.getAnnounceMessage(), player.getName(), "+" + item.getEnchantLevel() + " " + item.getName()) + " ::"); 			
			}
			else // fail
			{
				...
				if (!enchant.isCrystalize())
				{
					if (item.getEnchantLevel() < enchant.getDecrease())
					{
						player.sendPacket(EnchantResult.CANCELLED);
						return;
					}
					
					if (enchant.getDecrease() == 0 && enchant.getBreak() == -1)
					    player.sendMessage("Golden Enchant Failed. The enchant remained.");
					else // keep item
						player.sendMessage("Blessed Enchant Failed. The enchant value became " + enchant.getBreak()+ "."); // player.sendPacket(SystemMessageId.BLESSED_ENCHANT_FAILED);
					
					// get decrease enchant
					if (enchant.getBreak() != -1 || enchant.getDecrease() != -1)
					{
						item.setEnchantLevel(enchant.getDecrease() != -1 ? item.getEnchantLevel() - enchant.getDecrease() : enchant.getBreak());
						item.updateDatabase();
					}
