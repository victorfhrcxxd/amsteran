...
public final class RequestJoinParty extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (CTFEvent.isPlayerParticipant(target.getObjectId()) || FOSEvent.isPlayerParticipant(target.getObjectId()) || DMEvent.isPlayerParticipant(target.getObjectId()) || LMEvent.isPlayerParticipant(target.getObjectId()) || TvTEvent.isPlayerParticipant(target.getObjectId()) || MultiTvTEvent.isPlayerParticipant(target.getObjectId()))
		{
			requestor.sendPacket(SystemMessageId.YOU_HAVE_INVITED_THE_WRONG_TARGET);
			return;
		}
        
        if (target.isInsideZone(ZoneId.HIDE))
        {
            requestor.sendMessage("The player you tried to invite is inside a no party zone.");
            return;
        }
        
        if (requestor.isInsideZone(ZoneId.HIDE))
        {
            requestor.sendMessage("You cannot invite when you are inside solo event.");
            return;
        }
		...
		{
			requestor.sendMessage("You can not invite a player on another instance.");
			return;
		}
		...
		if (ClassPartyLimiter.getInstance().checkRequestorClassLimit(requestor, target))
			return;
