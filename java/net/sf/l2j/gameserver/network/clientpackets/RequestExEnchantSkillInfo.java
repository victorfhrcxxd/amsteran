...
public final class RequestExEnchantSkillInfo extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		for (L2EnchantSkillLearn esl : SkillTreeTable.getInstance().getAvailableEnchantSkills(activeChar))
		{
			...
			{
				if (esl.getId() == _skillId && (_skillLevel == 115 || _skillLevel == 155))
				{
					...
					ExEnchantSkillInfo esi = new ExEnchantSkillInfo(_skillId, _skillLevel, data.getCostSp(), data.getCostExp(), 100);
					esi.addRequirement(4, 9508, 1, 0);
					sendPacket(esi);
					
					break;
				}
			}
			else if (esl.getId() == _skillId && esl.getLevel() == _skillLevel)
			{
				L2EnchantSkillData data = SkillTreeTable.getInstance().getEnchantSkillData(esl.getEnchant());
				// Enchant skill or enchant data not found.
				if (data == null)
