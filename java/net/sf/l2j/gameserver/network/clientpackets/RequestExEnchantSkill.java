...
public final class RequestExEnchantSkill extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (activeChar.isMaxEnchantSkillsList())
		{
			// Try to find enchant skill.
		...
		if (activeChar.isMaxEnchantSkillsList())
		{
			if (!activeChar.destroyItemByItemId("MaxSkillEnchant", 9508, 1, null, true))
			{
				activeChar.sendPacket(SystemMessageId.YOU_DONT_HAVE_ALL_OF_THE_ITEMS_NEEDED_TO_ENCHANT_THAT_SKILL);
				return;
			}
		}
		else if (Config.ES_SP_BOOK_NEEDED)
		{
		...
		if ((activeChar.isMaxEnchantSkillsList() && (_skillLevel == 115 || _skillLevel == 155)) || Rnd.get(100) <= data.getRate(activeChar.getLevel()))
		{
		...
		if (activeChar.isMaxEnchantSkillsList())
			L2NpcInstance.showEnchantSkillList(activeChar);
		else
