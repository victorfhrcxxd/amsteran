		...
		public void run()
		{
			...
			try
			{
				if (getActiveChar() != null && !isDetached())
				{
					...
					if (Config.FAKE_OFFLINE && getActiveChar().isInsideZone(ZoneId.FAKE_ONLINE_ZONE))
					{
						getActiveChar().setFakeOffline(true);
						return;
					}

					if (Config.NO_CARRIER_SYSTEM_ENABLED && getActiveChar().isInFunEvent())
					{
						getActiveChar().scheduleDelete();
						return;
					}
					...
					// Offline Farm - keep player in game
					if (Config.ENABLE_OFFLINE_FARM && getActiveChar().isOfflineFarm())
					{
						getActiveChar().leaveParty();
						OlympiadManager.getInstance().unRegisterNoble(getActiveChar());
		...
		public void run()
		{
			try
			{
				...
				if (getActiveChar() != null) // this should only happen on connection loss
				{
					...
					{
						getActiveChar().setXYZ(ArenaTask.loc1x(), ArenaTask.loc1y(), ArenaTask.loc1z());
