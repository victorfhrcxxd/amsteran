...
public class DoorTable
{
					...
					for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
					{
						if (d.getNodeName().equalsIgnoreCase("door"))
						{
							...
							for (Node c = d.getFirstChild(); c != null; c = c.getNextSibling())
							{
								...
								else if ("coordinates".equalsIgnoreCase(c.getNodeName()))
 								{
									for (Node loc = d.getFirstChild(); loc != null; loc = loc.getNextSibling())
									{
							...
							door.setMapRegion(MapRegionTable.getMapRegion(posX, posY));
							door.setOpen(false);
	...
	private void onStart()
	{
		try
		{
			...
			// Beast Farm
			getDoor(21150003).openMe();
			getDoor(21150004).openMe();
			...
			// Core
			getDoor(20210001).openMe();
			getDoor(20210002).openMe();
			getDoor(20210003).openMe();
			
			// Opens all doors specified in configs for tvt
			if (Config.OPEN_DOORS_ENABLED)
				openDoors(Config.DOORS_IDS_TO_OPEN_LIST);
	...
	private void openDoors(List<Integer> doors)
	{
		for (int doorId : doors)
		{
			L2DoorInstance doorInstance = getDoor(doorId);
