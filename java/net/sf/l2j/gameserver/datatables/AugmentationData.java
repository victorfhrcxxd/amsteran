...
public class AugmentationData
{
	...
	// basestats2
	private static final int XBASESTAT_STR = 16345;
	private static final int XBASESTAT_CON = 16346;
	private static final int XBASESTAT_INT = 16347;
	private static final int XBASESTAT_MEN = 16348;
	
	// basestats3
	private static final int YBASESTAT_STR = 16349;
	private static final int YBASESTAT_CON = 16350;
	private static final int YBASESTAT_INT = 16351;
	private static final int YBASESTAT_MEN = 16352;
	
	// basestats4
	private static final int WBASESTAT_STR = 16353;
	private static final int WBASESTAT_CON = 16354;
	private static final int WBASESTAT_INT = 16355;
	private static final int WBASESTAT_MEN = 16356;
	
	// basestats5
	private static final int TBASESTAT_STR = 16357;
	private static final int TBASESTAT_INT = 16359;
	private static final int TZBASESTAT_STR = 16361;
	private static final int TXBASESTAT_STR = 16365;
	
	// Ori
	private static final int ORIGINAL_BASESTAT_STR = 16377;
	private static final int ORIGINAL_BASESTAT_CON = 16378;
	private static final int ORIGINAL_BASESTAT_INT = 16379;
	private static final int ORIGINAL_BASESTAT_MEN = 16380;
	...
	public L2Augmentation generateAugmentationWithSkill(int id, int level)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			if (_blueSkills[i].contains(stat34))
			{
				resultColor = 1;
				lifeStoneLevel = i;
				break;
			}
			else if (_redSkills[i].contains(stat34))
			{
				resultColor = 2;
				lifeStoneLevel = i;
				break;
			}
			else if (_purpleSkills[i].contains(stat34))
			{
				resultColor = 3;
				lifeStoneLevel = i;
				break;
		...
		switch (lifeStoneGrade)
		{
			case AbstractRefinePacket.GRADE_NONE:
				...
				if (!generateSkill && Rnd.get(1, 100) <= Config.AUGMENTATION_NG_BASESTAT_CHANCE)
					stat34 = Rnd.get(ORIGINAL_BASESTAT_STR, ORIGINAL_BASESTAT_MEN);
				break;
	...
	public List<AugStat> getAugStatsById(int augmentationId)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			// its a base stat
			else if (stats[i] >= BASESTAT_STR && stats[i] <= ORIGINAL_BASESTAT_MEN)
			{
				switch (stats[i])
				{
					case BASESTAT_STR:
					...
					case TXBASESTAT_STR:
						temp.add(new AugStat(Stats.STAT_STR, 1.0f));
						break;
					case ORIGINAL_BASESTAT_STR:
						temp.add(new AugStat(Stats.STAT_STR, 1.0f));
						break;
					case ORIGINAL_BASESTAT_CON:
						temp.add(new AugStat(Stats.STAT_CON, 1.0f));
						break;
					case ORIGINAL_BASESTAT_INT:
						temp.add(new AugStat(Stats.STAT_INT, 1.0f));
						break;
					case ORIGINAL_BASESTAT_MEN:
						temp.add(new AugStat(Stats.STAT_MEN, 1.0f));
						break;
				}
			}
