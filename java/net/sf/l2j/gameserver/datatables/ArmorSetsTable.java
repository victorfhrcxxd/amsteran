		...
		try
		{
			...
			if (!sets.isEmpty())
			{
				for (StatsSet set : sets)
				{
					...
					int enchant22skill = set.getInteger("enchant22skill", 0);
					_armorSets.put(chest, new ArmorSet(chest, legs, head, gloves, feet, skill_id, shield, shield_skill_id, enchant6skill, enchant22skill));
				}
			}
		}
		catch (Exception e)
