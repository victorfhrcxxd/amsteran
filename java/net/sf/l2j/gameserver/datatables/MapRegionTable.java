...
public class MapRegionTable
{
	...
	public Location getTeleToLocation(L2Character activeChar, TeleportWhereType teleportWhere)
	{
		if (activeChar instanceof L2PcInstance)
		{
			...
			L2HeavyZone heavy = ZoneManager.getHeavyArea(player);
			if (heavy != null)
				return heavy.getSpawnLoc();

			// Checking if in Free Chaotic Zone
			L2ChaoticSelfZone chaos = ZoneManager.getChaos(player);
			if (chaos != null)
				return chaos.getSpawnLoc();
			
			// Checking if in Self Chaotic Zone
			L2SpecialBossZone specialBoss = ZoneManager.getSpecialBoss(player);
			if (specialBoss != null)
				return specialBoss.getSpawnLoc();
