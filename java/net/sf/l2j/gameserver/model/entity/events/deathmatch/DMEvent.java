...
public class DMEvent
{
		...
		switch (pos)
		{
		    case 1:
	    		for (RewardHolder reward : DMConfig.DM_EVENT_REWARDS_WIN_1ST)
	    		{
	    			...
	    			{
	    				if (activeChar.isVip())
	    					activeChar.addItem("DM Reward", reward.getRewardId(), Rnd.get(reward.getRewardMin(), reward.getRewardMax()) * Config.VIP_DROP_RATE, activeChar, true);
	    				else
	    					activeChar.addItem("DM Reward", reward.getRewardId(), Rnd.get(reward.getRewardMin(), reward.getRewardMax()), activeChar, true);
	    			}
	    		}
			    break;
		    case 2:
	...
	{
		return HwidManager.getInstance().validBox(activeChar, DMConfig.DM_EVENT_NUMBER_BOX_REGISTER, allParticipants().values(), false);
	}
}