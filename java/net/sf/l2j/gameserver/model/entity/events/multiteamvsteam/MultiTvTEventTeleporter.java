...
public class MultiTvTEventTeleporter implements Runnable
{
	 ...
	 * @param adminRemove
	 */
	public MultiTvTEventTeleporter(L2PcInstance playerInstance, int[] coordinates, boolean fastSchedule, boolean adminRemove)
	{
		_playerInstance = playerInstance;
		_coordinates = coordinates;
		_adminRemove = adminRemove;
		
		long delay = (MultiTvTEvent.isStarted() ? MultiTvTConfig.MULTI_TVT_EVENT_RESPAWN_TELEPORT_DELAY : MultiTvTConfig.MULTI_TVT_EVENT_START_LEAVE_TELEPORT_DELAY) * 1000;
	...
	public void run()
	{
		...
		if ((MultiTvTConfig.MULTI_TVT_EVENT_EFFECTS_REMOVAL == 0) || ((MultiTvTConfig.MULTI_TVT_EVENT_EFFECTS_REMOVAL == 1) && ((_playerInstance.getTeam() == 0) || (_playerInstance.isInDuel() && (_playerInstance.getDuelState() != Duel.DUELSTATE_INTERRUPTED)))))
		...
		if (_playerInstance instanceof FakePlayer && !MultiTvTEvent.isStarted())
		...
		if (MultiTvTEvent.isStarted() && !_adminRemove)
		{
			//_playerInstance.setTeam(MultiTvTEvent.getParticipantTeamId(_playerInstance.getObjectId()) + 1);
