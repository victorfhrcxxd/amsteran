...
public class FOSEvent
{
	...
	private static void SpawnArtifact()
	{
		...
		try
		{
			...
			_lastArtifactSpawn.setTitle("event");
			_lastArtifactSpawn._isFOS_Artifact = true;
			_lastArtifactSpawn.isAggressive();
			_lastArtifactSpawn.decayMe();
			_lastArtifactSpawn.spawnMe(_ArtifactSpawn.getLastSpawn().getX(), _ArtifactSpawn.getLastSpawn().getY(), _ArtifactSpawn.getLastSpawn().getZ());
	 ...
	 */
	public static final boolean checkForFOSSkill(L2PcInstance source, L2PcInstance target, L2Skill skill)
	{
		if (!isStarted())
			return true;
	...
	public static boolean isDoorAttackable(int id, L2Character attacker)
	{
		for (int doorId : FOSConfig.FOS_DOORS_ATTACKABLE)
		{
			...
			if (attacker instanceof L2PcInstance && ((L2PcInstance)attacker).isInFOSEvent())
				return true;
