...
public class PartyZoneManager implements Runnable
{
	...
	protected PartyZoneManager() 
	{
		_event = PartyZone.getInstance();
		if (Config.PARTY_ZONE_EVENT_ENABLED)
		{
			_state = EngineState.AWAITING;
			ThreadPoolManager.getInstance().scheduleAiAtFixedRate(this, 1000, 1000);
			_log.info("Farming Event: is active.");
		}
		else
		{
			_state = EngineState.INACTIVE;
			_log.info("Farming Event: is disabled.");
		} 
	}
	...
	public void run() 
	{
		if (_state == EngineState.AWAITING)
		{
			...
			for (String time : Config.PARTY_ZONE_INTERVAL)
			{
		...
		else if (_state == EngineState.ACTIVE) 
		{
			...
			{
			case 7200:
				Broadcast.gameAnnounceToOnlinePlayers("Party Farm Event: " + (_tick / 60) + " minute(s) until finished!");
				break;
			case 5400:
				Broadcast.gameAnnounceToOnlinePlayers("Party Farm Event: " + (_tick / 60) + " minute(s) until finished!");
				break;
			...
			case 180:
				...
				break;
			case 30:
				Broadcast.gameAnnounceToOnlinePlayers("Party Farm Event: " + _tick + " second(s) until finished!");
				break;
			case 3:
			case 4:
			case 5:
	...
	public void startEvent() 
	{
		...
		{
			Broadcast.gameAnnounceToOnlinePlayers("Party Farm Event: is Started!");
			Broadcast.gameAnnounceToOnlinePlayers("Party Farm Event: spawned Event Champions!");
			Broadcast.toAllOnlinePlayers(new ExShowScreenMessage("Party Farm Event is Started!", 3000, SMPOS.TOP_CENTER, false));
	...
	public void endEvent()
	{
		...
		{
			Broadcast.gameAnnounceToOnlinePlayers("Party Farm Event: is Finished!");
			Broadcast.gameAnnounceToOnlinePlayers("Party Farm Event: Event Champions disappeared!");
	...
	{
		int[] coord;
		for (int i = 0; i < Config.PART_ZONE_MONSTERS_EVENT_LOCS_COUNT; i++)
		{
			coord = Config.PART_ZONE_MONSTERS_EVENT_LOCS[i];
	...
	{
		final NpcTemplate template = NpcTable.getInstance().getTemplate(Config.PART_ZONE_MONSTERS_EVENT_ID.get(Rnd.get(Config.PART_ZONE_MONSTERS_EVENT_ID.size())));
	...
	{
		protected static final PartyZoneManager INSTANCE = new PartyZoneManager();
	}
}