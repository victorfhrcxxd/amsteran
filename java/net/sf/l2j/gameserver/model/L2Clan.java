...
public class L2Clan
{
	...
	public void broadcastToOnlineMembers(L2GameServerPacket... packets)
	{
		for (L2ClanMember member : _members.values())
		{
			if (member != null && member.isOnline())
			{
				...
				{
					if (member.getPlayerInstance().isNoCarrier() && (packet instanceof CreatureSay))
					{
						member.getPlayerInstance()._noCarrierMessagePackets.add((CreatureSay) packet);
					}
					else
					{
	...
	public boolean checkClanJoinCondition(L2PcInstance activeChar, L2PcInstance target, int pledgeType)
	{
		...
		if (target.isInOlympiadMode() || activeChar.isInOlympiadMode() || target.isOlympiadProtection() || activeChar.isOlympiadProtection())
		{
			activeChar.sendMessage(target.getName() + " is in Olympiad Mode!");
			return false;
		}
	...
	public boolean checkAllyJoinCondition(L2PcInstance activeChar, L2PcInstance target)
	{
		...
		if (Config.DISABLE_JOIN_IN_ALLY)
		{
			activeChar.sendMessage("You cannot add ally in your clan. Ally function is disabled.");
			return false;
		}
	...
	public void addRankingChampPoints(int points) 
	{
		loadRankingPoints();
		_champPoints += points;
		storeRankingPoints();
