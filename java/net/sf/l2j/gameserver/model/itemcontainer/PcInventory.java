...
public class PcInventory extends Inventory
{
	...
	public TradeItem[] getAvailableItems(TradeList tradeList)
	{
		...
		for (ItemInstance item : _items)
		{
			...
			{
				if (_owner.getFakeArmorObjectId() == item.getObjectId() || _owner.getFakeWeaponObjectId() == item.getObjectId())
					continue;
	...
	public ItemInstance addItem(String process, ItemInstance item, L2PcInstance actor, L2Object reference)
	{
		...
		if (actor != null && DollsTable.getInstance().isDollById(item.getItemId()))
			DollsTable.refreshAllDollSkills(actor);

		for (Entry<Integer, Integer> itemTime : Config.LIST_TIMED_ITEMS.entrySet())
		{
			if (item.getItemId() == itemTime.getKey())
				ItemTimeManager.getInstance().setTimed(item, itemTime.getValue());
		}
	...
	public ItemInstance destroyItem(String process, ItemInstance item, int count, L2PcInstance actor, L2Object reference)
	{
		...
		if (actor != null && DollsTable.getInstance().isDollById(itemId))
			DollsTable.refreshAllDollSkills(actor);
	...
	public ItemInstance dropItem(String process, ItemInstance item, L2PcInstance actor, L2Object reference)
	{
		...
		if (actor != null && item != null && DollsTable.getInstance().isDollById(item.getItemId()))
			DollsTable.refreshAllDollSkills(actor);
