...
public class L2MonsterInstance extends L2Attackable
{
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (killer != null)
		{
			...
			if (player != null)
			{
				if  (Config.MISSION_LIST_MONSTER.contains(getTemplate().getNpcId()) && Config.ACTIVE_MISSION_FARM)
				{				
					if (!player.checkMissions(player.getObjectId()))
						player.updateMissions();

					if (!(player.isFarmCompleted() || player.getFarmCont() >= Config.MISSION_FARM_COUNT))
						player.setFarmCont(player.getFarmCont() + 1);			               
				}

				if  (Config.MISSION_LIST_CHAMPION_MONSTER.contains(getTemplate().getNpcId()) && Config.ACTIVE_MISSION_CHAMPION)
				{				
					if (!player.checkMissions(player.getObjectId()))
						player.updateMissions();

					if (!(player.isChampionCompleted() || player.getChampionCont() >= Config.MISSION_CHAMPION_COUNT))
						player.setChampionCont(player.getChampionCont() + 1);			               
				}
				
				if (ClanRankingConfig.ENABLE_CLAN_RANKING)
				{
					if (player != null && ClanRankingConfig.CLAN_RANKING_CHAMPION_POINTS_KILLER.containsKey(Integer.valueOf(getNpcId()))) 
					{
						L2Clan clan = player.getClan();
						if (clan != null) 
						{
							int points = ClanRankingConfig.CLAN_RANKING_CHAMPION_POINTS_KILLER.get(Integer.valueOf(getNpcId())).intValue();
							clan.addRankingChampPoints(points);
						} 
					} 
				}
			}
