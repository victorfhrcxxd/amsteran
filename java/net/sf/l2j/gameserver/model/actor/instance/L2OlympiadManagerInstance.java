...
public class L2OlympiadManagerInstance extends L2NpcInstance
{
	...
	public void showChatWindow(L2PcInstance player, int val)
	{
		...
		switch (npcId)
		{
			...
			case 30109: // Olympiad managers custom
		...
		if (Olympiad._validationEnd == 0 || Olympiad._period == 0)
			html.replace("%olym%", "Olym comp over: " + numDays + " day(s), " + numHours + " hour(s), " + numMins + " minute(s).");
		else
			html.replace("%olym%", "Olym comp start: " + numDays + " day(s), " + numHours + " hour(s), " + numMins + " minute(s).");
			...
			switch (val)
			{
				case 2: // Show rank for a specific class, example >> Olympiad 1_88
					...
					if (classId >= 88 && classId <= 118)
					{
						...
						List<String> fight = Olympiad.getInstance().getClassLeaderFights(classId);
						...
						{
							reply.replace("%points" + index2 + "%", value);
							index2++;
							if (index2 > 10)
								break;
						}
						...
						{
							reply.replace("%fights" + index3 + "%", value);
							index3++;
							if (index3 > 10)
								break;
						}
						...
						for (; index2 <= 10; index2++)
							reply.replace("%points" + index2 + "%", "");
						
						for (; index3 <= 10; index3++)
							reply.replace("%fights" + index3 + "%", "");
