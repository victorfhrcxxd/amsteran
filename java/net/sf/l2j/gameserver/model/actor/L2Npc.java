...
public class L2Npc extends L2Character
{
	...
	//public static int COVID_MASK = 30118;
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					if (getNpcId() == 10019)
					{
						if (player.getInventory().getItemByItemId(COVID_MASK) == null && player.getWarehouse().getItemByItemId(COVID_MASK) == null)
						{
							player.sendPacket(new CreatureSay(getObjectId(), 0, "Brynn", "Hein " + player.getName() + " don't go out without mask!"));
							player.addItem("Covid Mask", COVID_MASK, 1, null, true);
						}
					}
					*/
	...
	public void onActionShift(L2PcInstance player)
	{
		...
		else if (this instanceof L2MonsterInstance || this instanceof L2RaidBossInstance || this instanceof L2GrandBossInstance || this instanceof L2FestivalMonsterInstance || this instanceof L2ChestInstance)
		{
			NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setFile("data/html/action_shift.htm");
			html.replace("%id%", getTemplate().getNpcId());
			html.replace("%lvl%", getTemplate().getLevel());
			html.replace("%name%", getTemplate().getName());
			html.replace("%hp%", (int) getCurrentHp());
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (killer instanceof L2PcInstance)
			if (_isKTBEvent && ((L2PcInstance) killer).isInKTBEvent())
