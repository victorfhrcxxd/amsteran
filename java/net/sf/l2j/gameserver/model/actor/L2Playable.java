...
public abstract class L2Playable extends L2Character
{
	...
	{
		if (!CTFEvent.onAction(player, getObjectId()) || !FOSEvent.onAction(player, getObjectId()) || !DMEvent.onAction(player, getObjectId()) || !LMEvent.onAction(player, getObjectId()) || !TvTEvent.onAction(player, getObjectId()) || !MultiTvTEvent.onAction(player, getObjectId()))
			return;
	...
	public boolean doDie(L2Character killer)
	{
		...
		else if (isInsideZone(ZoneId.FLAG_AREA_SELF))
		{
			if (Config.CHAOTIC_LEAVE_BUFFS_ON_DIE)
				stopAllEffectsExceptThoseThatLastThroughDeath();

			if (Config.GET_SELF_ANNOUNCE)
				AnnounceSelfPlayer(getActingPlayer());
		}
	...
	private void AnnounceSelfPlayer(L2PcInstance player)
	{
		...
		if (!player.isGM())
		{
			if (player.getClan() != null)
				Broadcast.ServerAnnounce(player.getName() +" of the Clan " + player.getClan().getName() + " " + Config.GET_SELF_MSG + " ::");
			else
				Broadcast.ServerAnnounce(player.getName() +" " + Config.GET_SELF_MSG + " ::");
		}
	}
