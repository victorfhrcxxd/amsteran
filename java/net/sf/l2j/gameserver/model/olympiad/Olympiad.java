...
public class Olympiad
{
	...
	private static final String GET_EACH_CLASS_LEADER_CURRENT_POINTS = "SELECT olympiad_points from olympiad_nobles, characters WHERE characters.obj_Id = olympiad_nobles.char_id AND olympiad_nobles.class_id = ? AND olympiad_nobles.competitions_done >= 1 ORDER BY olympiad_nobles.olympiad_points DESC, olympiad_nobles.competitions_done DESC LIMIT 10";
	private static final String GET_EACH_CLASS_LEADER_CURRENT_FIGHTS = "SELECT competitions_done from olympiad_nobles, characters WHERE characters.obj_Id = olympiad_nobles.char_id AND olympiad_nobles.class_id = ? AND olympiad_nobles.competitions_done >= 1 ORDER BY olympiad_nobles.olympiad_points DESC, olympiad_nobles.competitions_done DESC LIMIT 10";
		...
		switch (Config.ALT_OLY_PERIOD)
		{
		    ...
		    { 
				...
				if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY)
					currentTime.add(Calendar.DAY_OF_WEEK, 8);
				
				if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY)
					currentTime.add(Calendar.DAY_OF_WEEK, 7);
				
				if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.TUESDAY)
					currentTime.add(Calendar.DAY_OF_WEEK, 6);
				
				if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.WEDNESDAY)
					currentTime.add(Calendar.DAY_OF_WEEK, 5);
				
				if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.THURSDAY)
					currentTime.add(Calendar.DAY_OF_WEEK, 4);
				
				if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.FRIDAY)
					currentTime.add(Calendar.DAY_OF_WEEK, 3);
				
				if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY)
					currentTime.add(Calendar.DAY_OF_WEEK, 2);
	...
	public List<String> getClassLeaderPoits(int classId)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement(GET_EACH_CLASS_LEADER_CURRENT_POINTS);
			statement.setInt(1, classId);
			ResultSet rset = statement.executeQuery();
	...
	public List<String> getClassLeaderFights(int classId)
	{
		List<String> names = new ArrayList<>();
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement(GET_EACH_CLASS_LEADER_CURRENT_FIGHTS);
			statement.setInt(1, classId);
			ResultSet rset = statement.executeQuery();
