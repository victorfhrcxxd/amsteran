...
public class L2Party
{
	...
	private final int _partyObjId;
	
	public int getPartyObjId()
	{
		return _partyObjId;
	}
	...
	public L2Party(L2PcInstance leader, int itemDistribution)
	{
		...
		_partyObjId = IdFactory.getInstance().getNextId();
		L2World.getInstance().addPartyToWorld(this);
	}
	...
	public void broadcastToPartyMembers(L2GameServerPacket packet)
	{
		for (L2PcInstance member : _members)
		{
			if (member != null)
			{
				if (member.isNoCarrier() && (packet instanceof CreatureSay))
					member._noCarrierMessagePackets.add((CreatureSay) packet);
				else
	...
	public void updateNoCarrierParty(L2PcInstance player)
	{
		...
		broadcastToPartyMembers(new CreatureSay(0, Say2.PARTY, "No Carrier", "Player: " + player.getName() + " has logged in again."));
	...
	public void broadcastCSToPartyMembers(CreatureSay msg, L2PcInstance broadcaster)
	{
		for (L2PcInstance member : _members)
		{
			...
			{
				if (member.isNoCarrier() && (msg instanceof CreatureSay))
				{
					member._noCarrierMessagePackets.add((CreatureSay) msg);
				}
				else
				{
