...
public final class Formulas
{
	...
	public static final double calcPhysDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean crit, boolean ss)
	{
		...
		if (crit)
		{
			if (Config.ENABLE_CUSTOM_PHYSICAL_CRITICAL_POWER && attacker instanceof L2PcInstance)
				damage = Config.PHYSICAL_CRITICAL_POWER * attacker.calcStat(Stats.CRITICAL_DAMAGE, 1, target, skill) * target.calcStat(Stats.CRIT_VULN, 1, target, null) * (70 * damage / defence);
			else
				damage = 2 * attacker.calcStat(Stats.CRITICAL_DAMAGE, 1, target, skill) * target.calcStat(Stats.CRIT_VULN, 1, target, null) * (70 * damage / defence); // Finally retail like formula

			damage += (attacker.calcStat(Stats.CRITICAL_DAMAGE_ADD, 0, target, skill) * 70 / defence); // Crit dmg add is almost useless in normal hits...
		}
		else
	...
	public static final double calcMagicDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean ss, boolean bss, boolean mcrit)
	{
		...
		{
			if (Config.ENABLE_CUSTOM_MAGE_CRITICAL_POWER)
	...
	public static boolean calcEffectSuccess(L2Character attacker, L2Character target, EffectTemplate effect, L2Skill skill, byte shld, boolean bss)
	{
		...
		if (Config.HERO_WEAPON_SKILLS_DEBUFF_CHANCE)
		{
	...
	public static float getChanceMultiplier(L2Skill skill)
	{
		...
		if (skill != null && skill.getSkillType() != null)
		{
			...
			if (Config.HERO_SKILL_MODIFIER_LIST.contains(skillid))
				multiplier *= Config.HERO_SKILL_WEAPON_DEBUFF_CHANCE_MODIFIER;
		}
