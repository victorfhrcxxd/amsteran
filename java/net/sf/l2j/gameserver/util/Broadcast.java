...
public final class Broadcast
{
	...
	public static void toAllOnlinePlayersPvpAnnounce(L2GameServerPacket mov)
	{
		for (L2PcInstance player : L2World.getInstance().getAllPlayers().values())
		{
			if (player.isOnline() && !player.isHideMsg())
				player.sendPacket(mov);
		}
	}
	
	public static void toAllOnlinePlayersQuakeAnnounce(L2GameServerPacket mov)
	{
		for (L2PcInstance player : L2World.getInstance().getAllPlayers().values())
		{
			if (player.isOnline() && !player.isHideMsg())
	...
	{
		toAllOnlinePlayersQuakeAnnounce(new CreatureSay(0, 15, ":", text));
	}
	
	public static void ServerAnnounce(String text)
	{
		toAllOnlinePlayers(new CreatureSay(0, 16, ":", text));
	}
    ...
    public static void announceToPlayersPvpAnnounce(String message)
    {
        for (L2PcInstance player : L2World.getInstance().getAllPlayers().values())
        {
        	if (!player.isHideMsg())
    ...
    public static void announceEventHideToPlayers(String message)
    {
        for (L2PcInstance player : L2World.getInstance().getAllPlayers().values())
        {
            player.sendHideWinnerMessage(message);
        }
    }

    public static void toSelfAndKnownPlayersFiltered(L2Character character, L2GameServerPacket packet, Predicate<L2Character> filter) 
    {
    	sendPacketFiltered(character, packet, filter);
    	character.getKnownList().getKnownType(L2PcInstance.class).forEach(player -> sendPacketFiltered((L2Character)player, packet, filter));
    }

    private static void sendPacketFiltered(L2Character character, L2GameServerPacket packet, Predicate<L2Character> filter)
    {
    	if (character instanceof L2PcInstance && (filter == null || filter.test(character)))
